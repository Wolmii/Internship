...
instance Randomizable RnnHypParams RnnParams where -- initialisation 
...
rnnLayers :: RnnParams -- ^ parameters (=model)
  ...
  -> Maybe Double    -- ^ introduces a Dropout layer on the outputs of each LSTM layer except the last layer, with dropout probability equal to dropout. to not overheat
...
rnnLayers RnnParams{..} actname dropoutProb h0 inputs = unsafePerformIO $ do
  ...
  let bidirectional | dnumLayers == numLayers * 2 = True
      ...
      dropoutLayer = case dropoutProb of -- for overheat 
      ...
      output = head outputList  -- | [<seqLen, D*oDim>] of length numLayers -- the returnss
      rh = hn                                      -- | 
