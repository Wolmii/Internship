...
cost ::
    ...
    Tensor    -- ^ loss: scalar
cost z z' = (1/(2*length z)) * (sum z' - sum z)
...
main :: IO ()
main = do
    let sampleA = asTensor(0.555)
    let sampleB = asTensor(94.585026)
  ...
  foldl(x -> print "correct answer: " + ys[0..] + "\n" + "estimated: " + linear (sampleA, sampleB) asTensor(x) + "\n" + "******" ) [] xs
   -- convertToTensor x
